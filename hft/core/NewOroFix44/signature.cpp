...
int password_cb(char *buf, int size, int , void *userdata) {
  ...
  int len = std::min<int>(strlen(password), size - 1);
  memcpy(buf, password, len);
  buf[len] = '\0';
  return len;
}
EVP_PKEY* Util::load_ed25519(const char* pem, const char* password)
// PEM → EVP_PKEY*
{
...
int Util::verify(const std::string& payload, const std::string& signature,
  ...
  {
    BIO* b64 = BIO_new(BIO_f_base64());
    BIO* mem = BIO_new_mem_buf(signature.data(), signature.size());
    BIO_set_flags(b64, BIO_FLAGS_BASE64_NO_NL);
    BIO* bio = BIO_push(b64, mem);

    sig_bin.resize(64);
    int n = BIO_read(bio, sig_bin.data(), 64);
    sig_bin.resize(n);            // 정상이라면 n == 64
    BIO_free_all(bio);
  }
  ...
  int ok = EVP_DigestVerify(
              ctx,
              sig_bin.data(), sig_bin.size(),     // ← 바이너리 서명
              reinterpret_cast<const unsigned char*>(payload.data()),
              payload.size());
  EVP_MD_CTX_free(ctx);
  return ok;
