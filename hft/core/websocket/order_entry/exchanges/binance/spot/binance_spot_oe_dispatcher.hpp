...
#ifndef BINANCE_SPOT_OE_DISPATCHER_HPP
#define BINANCE_SPOT_OE_DISPATCHER_HPP
...
struct BinanceSpotOeDispatchRouter {
  ...
  static constexpr std::optional<std::string_view> get_dispatch_type(const T& msg) {
    ...
    if constexpr (std::is_same_v<MsgType, schema::ExecutionReportResponse>) {
      ...
      return "8";
    } else if constexpr (std::is_same_v<MsgType, schema::SessionLogonResponse>) {
      return "A";
    ...
    if constexpr (std::is_same_v<MsgType, schema::CancelAndReorderResponse> ||
                  ...
                  std::is_same_v<MsgType, schema::OutboundAccountPositionEnvelope>) {
      return std::nullopt;
    }
    return std::nullopt;
  }
 ...
 private:
  ...
  static void handle_modify_order_response(
      ...
      const ContextType& context,
      const typename ExchangeTraits::WireMessage& /*message*/) {
    if constexpr (std::is_same_v<typename ExchangeTraits::ModifyOrderResponse, std::monostate>) {
      (void)response;
      (void)context;
      return;
    } else {
...
#endif  // BINANCE_SPOT_OE_DISPATCHER_HPP
